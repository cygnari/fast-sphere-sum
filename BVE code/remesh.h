...
// void remesh_points(vector<double>& curr_state, vector<double>& target_points, vector<vector<int>>& triangles, vector<vector<int>>& vert_tris, int point_count, int tri_count, double omega, run_config config1) {
//     vector<double> curr)target, v1, v2, v3;
...
void regrid_points2(vector<double>& curr_state, vector<double>& target_points, vector<vector<int>>& triangles, vector<vector<int>>& vert_tris, int point_count, int tri_count, double omega, int lb, int ub, int ID) { // remesh back to original particle locations
    vector<double> curr_target, curr_pos, v1, v2, v3; // need to fix vorticity remeshing
    vector<int> poss_tris;
    vector<double> baryco;
    // double curr_vor;
    int test_count, iv1, iv2, iv3;
    int success = 0, failure = 0, bad = 0;
    bool found;
    for (int i = lb; i < ub; i++) {
        ...
        for (int j = 0; j < test_count; j++) { // first check triangles adjacent to initial point
            ...
            v2 = slice(curr_state, 5 * iv2, 1, 3);
            v3 = slice(curr_state, 5 * iv3, 1, 3);
            baryco = barycoords(v1, v2, v3, curr_target);
            // for (int k = 0; k < 3; k++) cout << ba
            // cout << baryco[0] << " " << baryco[1] << " " << baryco[2] << endl;
            if (check_in_tri(v1, v2, v3, curr_target)) {
                tri_interp(iv1, iv2, iv3, v1, v2, v3, curr_state, target_points, curr_target, i, omega);
                success += 1;
                found = true;
