...
__global__ void copy_cuda(double* x, double* y, int n) { // copies y into x
    int id = blockIdx.x*blockDim.x + threadIdx.x;
    if (id < n) x[id] = y[id];
}

__device__ void dot_prod_cuda(double* x, double* y, int size, double* result) {
    result[0] = 0;
    for (int i = 0; i < size; i++) result[0] += x[i] * y[i];
}

__device__ void vec_norm_cuda(double* x, int size, double* result) {
    dot_prod_cuda(x, x, size, result);
    result[0] = sqrt(result[0]);
}

__device__ void vec_minus_cuda(double* x, double* y, int size) {
    for (int i = 0; i < size; i++) x[i] -= y[i];
}

__global__ void vec_add_cuda(double* x, double* y, int n) { // adds y to x in place, modifies x
    int id = blockIdx.x*blockDim.x + threadIdx.x;
    if (id < n) x[id] += y[id];
}

__device__ void vec_add_cuda_dev(double* x, double* y, int n) { // adds y to x
    for (int i = 0; i < n; i++) x[i] += y[i];
}

__global__ void scalar_mult_cuda(double* x, double scalar, int n) { // multiplies x by scalar in place, modifies x
    int id = blockIdx.x*blockDim.x + threadIdx.x;
    if (id < n) x[id] *= scalar;
}

__device__ void scalar_mult_cuda_dev(double* x, double scalar, int n) { // multiplies x by scalar in place, modifies x
    for (int i = 0; i < n; i++) x[i] *= scalar;
}

__device__ void slice_cuda(double* x, double* y, int start, int stride, int length) { // returns a slice of length from x, starting at start, every stride elements, in y
    for (int i = 0; i < length; i++) y[i] = x[start + i * stride];
}

__device__ void cross_product_cuda(double* x, double* y, double* result) { // cross product of x and y, in result
...
__device__ void project_to_sphere_cuda(double* p1, double radius) {
    double norm;
    vec_norm_cuda(p1, 3, &norm);
    for (int i = 0; i < 3; i++) p1[i] *= radius / norm;
}
...
__device__ void BVE_gfunc_cuda(double* x, double* y, double* result) { // modifies y in place
    double dot;
    dot_prod_cuda(x, y, 3, &dot);
    double denom = 1 - dot;
    cross_product_cuda(x, y, result);
    scalar_mult_cuda_dev(result, 1 / denom, 3);
}
...
// void points_assign(vector<vector<vector<int>>>& tri_verts, vector<vector<double>>& verts, vector<double>& points, vector<vector<vector<int>>>& tri_points, vector<vector<int>>& point_locs, int levels, int point_count) {
//     vector<double> v1;
//     vector<double> v2;
//     vector<double> v3;
//     vector<double> point;
